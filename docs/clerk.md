...
This setup is linked to the Clerk application `app_3EBz4XEZgGSoWCxMAueUbqOsWS3`. Always
pass `--app app_3EBz4XEZgGSoWCxMAueUbqOsWS3` to `clerk init` so the project links to
the right Clerk app.

If this is an existing project, run:

```bash
clerk init --app app_3EBz4XEZgGSoWCxMAueUbqOsWS3
```
...
```bash
clerk init --framework <framework> --pm <package-manager> --app app_3EBz4XEZgGSoWCxMAueUbqOsWS3
```
...
- Next.js proxy matchers include `'/__clerk/(.*)'` after
  `'/(api|trpc)(.*)'`
